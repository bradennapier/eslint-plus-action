...
function saveArtifacts(data, contents) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        yield downloadAllArtifacts();
        yield fs_1.promises.writeFile(`/action/.artifacts/${data.runId}`, contents);
        yield cache.saveCache([`/action/.artifacts`], constants_1.CACHE_KEY);
    });
}
exports.saveArtifacts = saveArtifacts;
function downloadAllArtifacts() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const result = yield cache.restoreCache(['/action/.artifacts'], constants_1.CACHE_KEY);
        if (!result) {
            console.log('No Cache Found, Creating Now');
            yield fs_1.promises.mkdir('/action/.artifacts', { recursive: true });
            console.log('Cache Save Result: ', yield cache.saveCache([`/action/.artifacts`], constants_1.CACHE_KEY));
        }
